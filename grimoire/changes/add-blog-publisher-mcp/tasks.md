...
- [x] 2.3 [TDD] git commit＋push 與 URL 回傳（PUB-R-05、R-06、R-11）：英文 commit message 沿用 `doc: add new article "<title>"` 慣例、push origin master 禁止 force、成功回傳預期上線 URL（`hugo.toml:27` permalink 規則）；push 被拒回 E-PUSH-REJECTED、本地 commit 保留｜驗證：`npm test` 全綠，git 操作以暫存 repo fixture 測試，含 EX-D#2 non-fast-forward case (blocked-by: 2.2; tier: 同級)
