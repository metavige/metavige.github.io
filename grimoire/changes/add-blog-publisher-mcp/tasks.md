...
- [ ] 1.1 安裝與 CI 同版的 hugo **extended 0.164.0**（`hugo.yml:30` 鎖定；brew 版本不符就改下載官方 release binary 進 PATH），並驗證整站可 build｜驗證：`hugo version` 輸出含 `0.164.0` 與 `extended`；repo 根目錄 `hugo --renderToMemory --quiet` exit 0 (tier: 中階)
- [ ] 1.2 查證 2026 當前最新 MCP spec 版本與官方 TypeScript SDK 最新 stable（官方文件優先：modelcontextprotocol.io 與 SDK repo；**實作前先查官方文件**），把 spec 版號、SDK package 名與版號、stdio server 最小範例出處 URL 記錄到本檔末「查證附註」段｜驗證：「查證附註」段存在且三項齊備、各附出處 URL (tier: 中階)
...
（由 task 1.2 填入：MCP spec 版號、TypeScript SDK package 與版號、stdio server 最小範例出處 URL）
